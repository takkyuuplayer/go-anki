...
const dictionaryName = "英辞郎 on the WEB"
...
func (dic *Eijiro) Parse(word, body string) (*dictionary.Result, error) {
	urlToSearch := template.URL(fmt.Sprintf(searchURL, url.QueryEscape(word)))
	...
	headword := text(cascadia.Query(definitions, cascadia.MustCompile("span.midashi > h2")))
	if headword != word {
	...
	first := cascadia.Query(definitions, cascadia.MustCompile("div > span.wordclass:nth-child(1)"))
	if first == nil {
		first = cascadia.Query(definitions, cascadia.MustCompile("div"))
		sense, examples := parseDefinition(first.FirstChild)
		dictEntry = dictionary.Entry{
			ID:              fmt.Sprintf("eijiro-%s", headword),
			Headword:        headword,
			FunctionalLabel: "",
			Definitions: []dictionary.Definition{
				{Sense: template.HTML(sense), Examples: examples},
			},
		}
