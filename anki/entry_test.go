...
func TestEntry_AnkiCard(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			entry := &Entry{
				ID:              tt.fields.ID,
				Headword:        tt.fields.Headword,
				FunctionalLabel: tt.fields.FunctionalLabel,
				Pronunciation:   tt.fields.Pronunciation,
				Inflections:     tt.fields.Inflections,
				Definitions:     tt.fields.Definitions,
			}

			if got, _ := entry.AnkiCard(); got != tt.want {
